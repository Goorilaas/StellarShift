...
StellarShift — це автоматична зміна шпалер, яка робить кожен ранок іншим.
...
• Десятки тисяч HD-фото з 20+ категорій: Космос, Природа, Аніме,
...
• Tens of thousands of HD photos across 20+ categories: Space, Nature,
  Anime, Cyberpunk, Architecture, Ocean, Minimal, B&W…
...
Темно-космічний фон (gradient `#0a0a1a` → `#1a1a2e` → `#3a2a5e`). Зліва — невелика сітка з 4-6 wallpaper-thumbnail'ів (різні категорії: галактика, гори, аніме, океан) із plus-1 «активною» в центрі сітки з підсвіткою #FFD700. Справа — назва **StellarShift** великим (Inter ExtraBold, 88px) + slogan «Космос у твоїй кишені і не тільки» (Inter Medium, 28px, opacity 0.7) + dot-pagination внизу.
