...
- [x] **Колекції / mood-паки** — зроблено у **v4.0.0** 🎉 (таб, 18 настроїв × ~100 колекцій, ♥+ротація, Model B; полір → 4.0.1). Початковий рефенс: — référенс Walpy: куровані добірки hero-картками (велике фото + «N Photos» + назва + куратор), всередині — грид. Наші: «Шлях чумацький», «Неонова ніч», «Тиша гір»…
- [x] **Серце прямо на тайлах гриду** (каталог + Улюблені) — зроблено у **v3.11.0**. Замість Walpy-обведення — наша унікальна «cosmic spark»: pop + розліт 4 бренд-зір. Long-press прибрано
- [x] **Тюнінг категорій** — зроблено у **v3.12.0**: фільтр людей на 7 категоріях + ротація 20+ точкових під-запитів (Гори/Космос/Океан/Тварини/Природа) проти повторів і «приїлих» геройських кадрів
- [x] **Щоденний фоновий перезбір пулу** — зроблено у **v3.13.0**: Worker сам раз на добу тягне свіжий контент + нову ротацію під-запитів (рецепт від JS, Kotlin-виконавець), без відкриття застосунку. Swap-on-success
- [x] **Author-cap + дощ-ротація** — зроблено у **v3.13.1**: ≤2 фото на автора (проти «шпалерних ферм» типу BoliviaInteligante, що флудять усі запити) + 20 під-запитів для Дощу
- [x] **Адаптивний subCount** — зроблено у **v3.13.2**: під-запитів на категорію залежить від розміру міксу (1–4→6 … 15+→2), малий мікс тягне багатше; каталожний Мікс теж розширено (~80+ замість 31)
- [x] **Ротація для ВСІХ категорій** — зроблено у **v3.14.0**: решта 16 категорій (абстракції/їжа/квіти/кіберпанк/міста/мінімал/сезони/ч-б/авто/яхти/архітектура/ліс/мандри/туман/нуар/сільське) отримали по 15 під-запитів. Кожна категорія крутить теми, не залипає на базовому запиті
...
### 🎨 UI/UX
- [ ] **Колекції: повернення в таб** — зараз switch tab → назад пам'ятає глибокий рівень (настрій/колекція) + відкритий PhotoViewer на весь екран. Лишити так чи скидати лише фуллскрін-фото (рівень тримати)? Юзер тестує, рішення згодом *(баг-репорт 2026-06-20)*
...
- [ ] **Розширити різноманіття пулу при вищих API-лімітах** — коли отримаємо збільшений ліміт Unsplash: підняти стелю запитів вище 44, розширити списки під-запитів понад 15 і брати більше на категорію (зараз 4–6). Переглянути banди `subCountForMix` під новий бюджет. *(Усі категорії вже мають ротацію — v3.14.0; лишається тільки підняти кількість.)*
- [ ] **Настрій «Дзен»** (Колекції, 4.1.0+) — окремий напрямок: йога / Тибет / буддизм / медитація. Зібрати куровані Unsplash-колекції під нього.
