...
### Виправлено
- Рядки підколекцій читають свіжі збережені прев’ю одразу після появи, без очікування 200 мс видимості та додаткового таймера 150 мс. Читання кешу не виконує API-запитів.
- Для порожнього/простроченого кешу збережено відкладене завантаження лише видимих рядків, спільні запити з фото-сіткою та TTL 24 години. Пізнє читання диска не перезаписує новіший кеш у пам’яті.
- Версія 4.0.13, Android versionCode 58; залежності не змінено.

### Перевірки й обмеження
- Пройшли 11 нових перевірок кешу й ефектів рядка та 6 перевірок приховування. TypeScript без помилок; lint — 0 помилок, 3 попередні попередження React Hooks.
- У тестах повторне відкриття й відновлення зі свіжого кешу після перезапуску не роблять додаткових API-запитів; вихід до завершення таймера скасовує заплановане завантаження.
- APK не збирався. Реальна швидкість відображення на S23 Ultra не вимірювалася. Перше відкриття без кешу, завантаження метаданих та зображень усе ще залежать від мережі.

## 4.0.12 — 2026-09-25

### Виправлено
- У колекціях встановлення шпалери починається з побажання, а після успіху закриває перегляд до тієї самої сітки. Повідомлення «Красу встановлено» показує сітка, тому закриття перегляду не прибирає його. При помилці фото залишається відкритим.
- Додавання в улюблені з перегляду колекції кнопкою або подвійним тапом дає легкий вібровідгук після збереження. Повторні натискання під час запису ігноруються; при помилці немає хибної анімації/вібрації, є повідомлення UA/EN.
- Плашки автора в каталозі, колекціях і улюблених компактні, з однаковим оформленням та обмеженням ширини перед кнопкою закриття; довгі імена скорочуються.
- Версія 4.0.12, Android versionCode 57.

### Перевірки й обмеження
- 44 JS-перевірки пройшли (6 нових сценаріїв перегляду); TypeScript без помилок; lint без помилок, 3 наявні попередження React Hooks.
- APK не збирався; вигляд плашок, показ повідомлень у native Modal і фізичний вібровідгук потребують перевірки на телефоні.
- Затримку мініатюр підколекцій діагностовано, але не змінено: видимість рядка 200 мс + таймер 150 мс перед читанням кешу/запитом, далі завантаження зображень. Оптимізація — окреме погодження. Ривки після довгої перерви залишаються відкритими.

## 4.0.11 — 2026-09-24

### Документація
- Об’єднано плани з Obsidian, історію робіт і результати перевірок у новий Roadmap: надійність, якість підзапитів, економія API, колекції, Samsung QA та власна бібліотека. Для задач визначено критерії й статуси, без обіцянок майбутніх версій/строків.
- Додано актуальний порядок розробки, версій і PR у `docs/development.md`; оновлено README та посилання в CLAUDE.md. Старі LAUNCH/RELEASE/STORE/history_speak позначені історичними.
- В Obsidian збережено ідентичний Roadmap, окремий QA-журнал, реєстр 29 кандидатів і спільну інструкцію випуску. Старі п’ять нотаток перенесено в датований архів без зміни вмісту; посилання Coding MOC актуалізовано.
- Відділено підтверджені тести 4.0.10 від неперевірених телефонних сценаріїв; старі обіцянки безлімітного API та гарантованого запуску не використовуються як чинний план.
- Версія 4.0.11, Android versionCode 56. Поведінка застосунку не змінюється; APK не збирається.

## 4.0.10 — 2026-09-24

### Змінено
- Каталог, пошук, «Мікс» і «Хаос» зберігають добірки на 24 години між запусками. Кеш обмежено 20 записами та 600 000 символами JSON; фото залишаються за оригінальними URL Unsplash.
- Кеш «Міксу» враховує вибрані категорії. Ручне оновлення, кнопка перемішування й повторний тап по активному «Міксу»/«Хаосу» запитують нову добірку.
- Кеш зберігає під-запит, сторінку й ознаку кінця результатів; пагінація продовжує правильну добірку, прибирає дублікати й зупиняється на останній сторінці.

### Виправлено
- Запізнілі відповіді та помилки попередньої категорії не замінюють поточні фото й не скидають індикатор нового завантаження.
- Повторні виклики під час того самого завантаження не створюють дубльованих запитів. Після помилки ручного оновлення поточна добірка залишається на екрані.
- Версія 4.0.10, Android versionCode 55. APK у межах цієї зміни не збирається.

## 4.0.9 — 2026-09-23

### Виправлено
- Приховані фото відфільтровано в колекціях, прев’ю та кешованих обкладинках. Скасування повертає фото з локального списку.
- Каталог, колекції та шторка Android використовують спільний список прихованих фото. Старий список переноситься при першому запуску.
- Приховування одразу очищує поточний пул автозміни; щоденний перезбір і фото, що ще завантажуються, також перевіряють актуальний список.
- Приховування, розблокування й Undo більше не запускають перезбір пулу через Unsplash. Undo очищення списку зберігає нові блокування зі шторки.

### Змінено
- Версія 4.0.9, Android versionCode 54. Через native-зміни потрібна нова Android-збірка.
- Регресійні перевірки: `node scripts/blocked.test.cjs`, `node scripts/rotation.test.cjs` та `bash scripts/test-blocked-native.sh` (JDK 17, Android SDK 36 і залежності з Gradle-кешу після Android-збірки).

## 4.0.8 — 2026-09-21

### Виправлено
- Вимкнення автозміни скасовує незавершені JS-запуски та Android-перезбір колекцій, щоб старе завантаження не відновлювало розклад.
- Активні колекції отримують вибрані інтервал і цільовий екран перед перезбором.
- Відкладені запуски використовують актуальні налаштування.

### Змінено
- Зміна лише інтервалу чи екрана повторно використовує наявний пул без запитів Unsplash, скидання позиції та таймера щоденного оновлення.
- Додано локальні регресійні тести керування автозміною: `node --test scripts/rotation.test.cjs`.
- Потрібна нова Android-збірка через зміни native-модуля. Перевірка на телефоні ще попереду.

## 4.0.7 — 2026-09-21

### Змінено
- Описи колекцій та списки фото кешуються на 24 години й зберігаються між запусками застосунку.
- Одночасні однакові запити без власного AbortSignal об’єднуються; кеш фото враховує сторінку та кількість фото.
- Прев’ю завантажуються лише для видимих рядків колекцій, зокрема після восьмого рядка. Прев’ю та фото-грид використовують спільний кеш.

### Перевірено
- Локальний тест із заміною мережі: відкриття тієї самої колекції після перезапуску — 0 запитів замість 2 за наявності свіжого кешу.
- TypeScript та ESLint змінених файлів проходять. APK і перевірка на телефоні — ще не виконані.

## 4.0.6 — 2026-06-20

🎭 **Паритет переглядачів фото.** JS-only.

### Змінено
- **Переглядач фото в колекціях = вигляд каталогу:** власні SVG-іконки (замість Ionicons), розкладка «В улюблені» + «Встановити» + 3 круглі (зберегти/поділитись/сховати), закрити справа-зверху, автор зліва.
- **Дабл-тап у колекціях → в улюблені + політ серця** (тільки додає, як у каталозі). Раніше жесту не було.
- **Тап по автору → інфо-картка** (біо + «відкрити профіль») у колекціях І в улюблених. Раніше в улюблених був прямий перехід на профіль.

### Додано
- **Кнопка «закрити» (✕) в улюблених** — раніше закривалось лише тапом по фото.
- **Верхній градієнт-scrim у переглядачі колекцій** — статус-бар і кнопки читаються на яскравому фото.

### Технічне
- **Інфо-модалка автора винесена у спільний `components/AuthorInfoModal`** — каталог/улюблені/колекції ділять один лист (мінус потрійне дублювання). Цілі переглядачі лишаються окремими (A1) — UI тримаємо в синхроні вручну.

## 4.0.5 — 2026-06-20

🧹 **Колекції: полір списку.** JS-only.

### Виправлено
- **Хедер деталі настрою / фото-гриду більше не лізе під статус-бар** — застосовано safe-area інсет, відступ зліва вирівняно на 16px (як на решті екранів).
- **Обкладинка настрою збігається з тим, що відкривається** — тапнута картка передає свій cover у герой-кадр деталі (раніше герой завжди брав першу колекцію → інша картинка).
- **Рядок колекції більше не «стрибає»** — місце під стрічку прикладів резервується одразу (плейсхолдери → реальні мініатюри), без зміни висоти при дозавантаженні.

### Змінено
- **Кнопка шпалери — однаковий напис «Встановити» скрізь** (каталог був «Встановити шпалери»). Уніфікація з улюбленими.

## 4.0.4 — 2026-06-15

🔍 **Колекції: приклади в рядках.** JS-only.

### Додано
- **У рядку кожної колекції — cover + стрічка 2-3 прикладів** (перші фото добірки). Видно одразу, що всередині, ще до тапу.
- **Кеш `getCollectionPhotos`** (page 1): приклади і фото-грид **ділять один фетч** — тапнув колекцію після прев'ю → фото вже там, миттєво.

### Технічне
- Приклади — **ліниво-cap** (перші 8 рядків настрою) + стаґер, щоб не вибухнути запитами. Кеш робить повторні відкриття безкоштовними.

## 4.0.3 — 2026-06-15

🎨 **Колекції: живі обкладинки.** JS-only.

### Змінено
- **Картки настроїв = реальні обкладинки** замість кольору: cover колекцій настрою + градієнт-затемнення під назвою. **Шафл на focus** — щоразу як відкриваєш таб, кожна картка показує випадкову обкладинку зі свого кешу. Поки вантажиться / якщо впало → fallback на колір+мотив.
- **Хедер деталі настрою = великий герой-кадр** (обкладинка першої колекції) замість плаского кольору.

### Технічне
- Диск-кеш обкладинок (`mood_covers`): Фаза 1 — перша обкладинка кожного настрою (18 запитів, кеш назавжди); решта **доростає органічно** при браузингу деталей (metas там і так тягнуться — нуль зайвих запитів). Безпечно для demo-ключа 50/год.
- Eager-prefetch усіх обкладинок (для BYO-ключа з вищим лімітом) — за потреби у 4.0.x.

## 4.0.2 — 2026-06-15

✨ **Колекції: повний перегляд фото.** JS-only.

### Додано
- **Тап по фото в колекції → фуллскрін-переглядач** з усіма діями, як у каталозі: **Встановити шпалеру**, ♥ Улюблене, Поділитись, Зберегти в галерею, 🚫 Сховати (вилітає з пулу автозміни). Логіка — **спільні сервіси** (share/save винесено в `services/photoActions`, каталог не чіпали).
- Швидкий ♥ просто на тайлі лишився.

> Візуальне збагачення карток настроїв (реальні обкладинки) — 4.0.3.

## 4.0.1 — 2026-06-15

🛠 **Колекції: навігація + фото-грид + видимість ротації.** JS-only.

### Виправлено
- **Жест «назад» з настрою** більше не викидає в Каталог — повертає на рівень настроїв (а з фото-гриду → у настрій). `BackHandler` по рівнях.

### Додано
- **Фото-грид колекції:** тап у колекцію (обкладинка/назва) → грид її фото → **♥ в улюблені просто з гриду**. Тепер видно, що саме обрав.
- **Чип «У ротації · N»** третім — активні колекції під рукою (бачиш що крутиться, що зняти).

> Візуальне збагачення карток настроїв + хедера (реальні обкладинки) — у 4.0.2.

## 4.0.0 — 2026-06-14

🎨✨ **Колекції** — куровані настрої замість пошуку. Хедлайнер 4.0.

> ⚠️ **Потребує повного rebuild'а** — Kotlin-зміни.

### Додано
- **Новий таб «Колекції».** 18 настроїв-напрямків (Шлях чумацький, Тиша гір, Неонова ніч, Глибінь, Дух гри, Golden hour, Alpenglühen, Скарбниця…). Настрій = швидка навігація (не нескінченний скрол), всередині — **куровані Unsplash-колекції з іменами авторів-кураторів**. Люди вже відсіяли шум і «ферми», з якими ми воювали keyword'ами.
- **Дві дії на колекції:** ♥ закладка (полиця «Мої колекції») + «в ротацію». Активних 1 чи багато → шпалери крутяться з їх **об'єднання**. **Model B:** активні колекції перебивають категорії; зняв усі → фолбек на категорії. Активація сама вмикає автозміну.
- **Фоновий перезбір поважає колекції** — щоденне оновлення тягне свіже з активних колекцій (`/collections/:id/photos`), author-cap + дедуп + шафл.

### Технічне
- ~100 курованих collection-ID у 18 настроях, зібрані гібридно (юзер знаходить + я скаутю). Метадані (обкладинка/куратор/к-сть) кешуються на сесію (береже 50/год ліміт).
- Полір (фото-грид колекції, міні-стрічки прев'ю, повний i18n) — у 4.0.1.

## 3.14.0 — 2026-06-14

🎲 **Ротація під-запитів для ВСІХ категорій** — кожна тема тепер жива.

### Додано
- **Решта 16 категорій отримали по 15 точкових під-запитів** (раніше ротацію мали лише 6): абстракції, їжа, квіти, кіберпанк, міста, мінімалізм, сезони, Ч/Б, авто, яхти, архітектура, ліс, мандри, туман, нуар, сільське. Тепер **кожна** категорія крутить різні теми замість одного банального запиту — у пулі автозміни, каталозі і фоновому перезборі.
- Ціль **~15** під-запитів на категорію — горизонт різноманіття ~2.5 дня навіть при найщільнішому міксі (subCount 6), і вища якість, ніж 20 «з натяжкою».

JS-only — рецепт фонового перезбору сам підхопить нові списки (Kotlin не чіпали).

## 3.13.4 — 2026-06-14

### Виправлено
- **Прибрано машини з «Дощу».** Під-запит `rain on car windshield` Unsplash читав за словом **car** → у Мікс лізли авто (жовте авто, салон з кермом). Замінено на 3 чисті: `rain over a calm lake`, `rainy forest path`, `downpour over city skyline` (дощ став 22 під-запити). Правило на майбутнє: у списках лише «дощ + явище/природа/вода», без слів-предметів (car/train/umbrella) — предмет перебиває «rain». JS-only.

## 3.13.3 — 2026-06-14

### Виправлено
- **Побажання (easter) в Settings тепер відцентроване.** Рядок іконка+фраза тиснувся вліво (`alignSelf: stretch` без `justifyContent`, текст `flex: 1`). Тепер `justifyContent: center` + текст `flexShrink: 1` / `textAlign: center` — фраза по центру, довгі переноси теж центровані. JS-only.

## 3.13.2 — 2026-06-14

🎨 **Адаптивна різношаровість Міксу** — менше категорій = більше варіантів кожній.

### Змінено
- **Кількість під-запитів на категорію тепер залежить від розміру міксу** (Варіант A), щоб укластись у бюджет ~44 запити (demo-ключ 50/год):
  - **1–4 категорії → 6** під-запитів кожній
  - 5–7 → 5 · 8–10 → 4 · 11–14 → 3 · 15+ → 2

  Раніше фіксовано (соло 5 / мікс 2). Тепер малий мікс (гори/космос/океан/дощ) тягне **6** під-запитів кожній — океан = хвилі/риф/острови/скелі/лагуна/барель, а не самі кити. Великий мікс сам опускається до 2 — бюджет цілий.
- **Каталожний Мікс** став багатшим (**~80+** фото замість ~31), з легшим cap ≤12 запитів, щоб гортання не палило ліміт.

JS-only — рецепт фонового перезбору сам підхопить нові числа (Kotlin не чіпали).

## 3.13.1 — 2026-06-14

🎯 **Менше повторів і «ферм»** — фікс одноманіття від одного автора.

> ⚠️ **Потребує повного rebuild'а** — Kotlin-зміни.

### Виправлено
- **Ліміт ≤2 фото на автора.** На Unsplash є «шпалерні ферми» — автори, що залили тисячі картинок і ранжуються майже на будь-який запит (напр. BoliviaInteligante), через що грид/пул ставав одноманітним: схожі калюжі, ті самі рендери, навіть абстракції в «океані». Дедуп тепер не лише по ID фото, а й по **автору** — у пулі автозміни, каталозі (Мікс/Хаос) і фоновому перезборі. Favorites — без ліміту (власні вибори юзера).

### Додано
- **Ротація під-запитів для «Дощу»** — 20 точкових варіантів (rainy window / storm clouds / neon rain / rain on leaves / monsoon / rain on ocean…) замість одного `wet street`, що давав ті самі калюжі. Раніше ротацію мали лише гори/космос/океан/тварини/природа — тепер і дощ.
