...
🔄 **Щоденний фоновий перезбір пулу** — свіжі шпалери щодня, не відкриваючи застосунок.

> ⚠️ **Потребує повного rebuild'а** — Kotlin-зміни.

### Додано
- **Пул автозміни сам оновлюється раз на добу** у фоні. Раніше пул збирався один раз і крутився ~2 тижні; тепер `WallpaperWorker` на плановому тіку перевіряє вік пулу і, якщо старший за 24год, **сам тягне свіжий контент** з Unsplash — з новою рандом-ротацією під-запитів (інші гори/космос/океан щодня). Відкривати застосунок не треба.
- **«Рецепт» замість дубля логіки**: JS пише в native prefs списки під-запитів + прапорці (excludePeople, chaos, people-keywords, blocked, favorites), Kotlin лише виконує — щодня сам рандомить ротацію. Списки запитів лишаються єдиним джерелом у `categories.ts`.
- **Swap-on-success**: якщо фетч впав (нема мережі / 403 / порожньо) — старий пул лишається недоторканим, ретрай за годину. Жодних порожніх екранів.

### Технічне
- Перезбір — лише на плановому тіку (ручні «Змінити зараз»/⏭ лишаються миттєвими). Фетч паралельний (coroutines), щоб ~44 запити не впёрлись у 10-хв ліміт WorkManager.
- Активується після наступної збірки пулу (увімкни автозміну або зміни категорію раз) — тоді рецепт запишеться і 24h-годинник стартує.

## 3.12.0 — 2026-06-14

🎨 **Тюнінг категорій** — менше повторів, менше людей там, де їх не просили.

### Додано
- **Ротація під-запитів** для «приїлих» категорій: **Гори (21), Космос (20), Океан (23), Тварини (20), Природа (20)** тепер крутять 20+ точкових запитів замість одного банального. Замість `mountains peaks` — `alpenglow on mountain peaks` / `dolomites` / `misty valley` / …; замість `space galaxy stars` — `milky way astrophotography` / `aurora borealis` / `orion nebula` / … Щоразу інший зріз → пул і каталог не залипають на тих самих геройських кадрах.
  - **Мікс**: 2 різних під-запити на категорію за збірку — бюджет запитів той самий.
  - **Соло-категорія**: 4-5 під-запитів (весь пул із неї однієї, тож беремо ширше).

### Змінено
- **Фільтр людей** увімкнено на **Горах, Космосі, Природі, Океані, Містах, Мандрах, Сільському** (раніше пропускали туристів/силуети/натовп). Ч/Б і Нуар свідомо лишаються з людьми — там стріт-фото це жанр. Фільтр keyword-based (по тегах/опису Unsplash), тож не ідеальний, але прибирає більшість.

## 3.11.2 — 2026-06-14

### Виправлено
- **Пул більше не перебудовується на кожен вхід у Settings.** Тост «N фото готові до автозміни» вискакував щоразу при відкритті налаштувань — навіть коли нічого не міняв. Причина: `loadSettings` на кожен focus робив `setState` новими масивами категорій (через `JSON.parse`), а ефект-перезавантаження порівнював залежності **за посиланням** → бачив фальшиву «зміну» → дарма ганяв зайві запити до Unsplash і рестартив ротацію. Тепер порівнюємо **за значенням** (ключ із категорій + інтервалу + цілі): збігається з уже застосованим — нічого не робимо. Економить demo-ключ (50 запитів/год) і прибирає набридливий тост. JS-only, без Kotlin.

## 3.11.1 — 2026-06-12

### Виправлено
- **Cosmic spark тепер видно.** Серце в кутку тайла, а тайл обрізає все за краєм (`overflow: hidden`) — у v3.11.0 зорі летіли радіально, тож половина (вгору/вправо) одразу зрізалась краєм, звідси «ледь помітно». Тепер 6 зір осипаються **віялом вниз-вліво** у видиму зону, **більші** (5px + 10px світіння-німб), тримаються довше (540мс), а серце вистрибує помітніше (нижчий friction → більший overshoot). Підпис нарешті сяє.

## 3.11.0 — 2026-06-12

💜✨ **Серце на тайлах** — улюблені в один тап прямо з гриду, з нашою фірмовою анімацією.

### Додано
- **Кнопка-серце на кожному тайлі** (каталог + Улюблені): біле порожнє серце на напівпрозорому темному колі (видно на будь-якому фоні, навіть світлих квітах/небі). Тап по серцю → в улюблені, **не відкриваючи фото**.
- **«Cosmic spark» — наша унікальна анімація** (не клон): при лайку червоне серце вистрибує (spring-pop), і з нього розлітаються **4 крихітні зорі** в бренд-палітрі (золота / бузкова / фіолетова), що віддаляються й тануть за ~420мс. В'яжеться з усім космічним брендом — зорі в благословеннях, star-burst на лого.
- **В Улюблених** серце завжди заповнене → тап швидко **знімає** з улюблених (з undo-тостом, як і раніше).
- Компонент `components/FavoriteHeart.tsx` — спільний для обох екранів, чистий `Animated` (без reanimated). На першому рендері — миттєвий стан без сплеску (скрол не «вибухає»).

### Змінено
- **Long-press на тайлі прибрано** — серце тепер видимий, дискаверабельний шлях у улюблені. Подвійний тап для лайку лишається в модалці перегляду (там він нічого не коштує). Одиночний тап по тайлу → відкрити фото, миттєво (без затримки на детект double-tap).
- Старий бейдж-серце (з'являвся лише коли вже улюблене) замінено новою завжди-видимою кнопкою.
